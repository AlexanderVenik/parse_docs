...
for index_paragraphs in range(len(doc.Paragraphs)):
    ...
    if r != prevPage: 
        if paragraph.Range.Text.strip() and paragraphFormat.Bold and prevPage != r:
            prevPage = r
            processStep(paragraphFormat.SpaceBefore, SPACE_BEFORE_HEADERS, pc(paragraphFormat.SpaceBefore, SPACE_BEFORE_HEADERS, 0.3), f"Отступ снизу после заголовка {index_paragraphs}")
...
cnt = pgs.Count
minimal = 40 + 3
processStep(cnt, minimal, cnt >= minimal, "Объем документа (предваритеьный)")
